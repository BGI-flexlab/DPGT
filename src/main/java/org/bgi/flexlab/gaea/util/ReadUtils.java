...
public final class ReadUtils {
    ...
    /**
     * Checks whether read is a headerless SAMRecordToGaeaSamRecordAdapter, and if it is, sets its
     * header to the provided header.
     *
     * @param read A potentially headerless GaeaSamRecord
     * @param header header to store in the read, if it's a headerless SAMRecord-backed read
     */
    public static void restoreHeaderIfNecessary( final GaeaSamRecord read, final SAMFileHeader header ) {
     ...
     */
    public static byte[] getOriginalBaseQualities( final GaeaSamRecord read ) {
     ...
     */
    public static String getBaseQualityString( final GaeaSamRecord read ) {
        Utils.nonNull(read);
     ...
     */
    public static void setBaseQualityString(final GaeaSamRecord read, final String baseQualityString) {
        Utils.nonNull(read);
     ...
     */
    public static boolean readHasNoAssignedPosition( final GaeaSamRecord read ) {
        ...
        return read.getReferenceName()== null ||
               read.getReferenceName().equals("*") ||
    ...
    public static SAMReadGroupRecord getSAMReadGroupRecord( final GaeaSamRecord read, final SAMFileHeader header ) {
        final String readGroupName = (String)read.getAttribute(SAMTagUtil.getSingleton().RG);
     ...
     *         or null if the read has no read group.
     */
    public static String getPlatform( final GaeaSamRecord read, final SAMFileHeader header ) {
     ...
     *         or null if the read has no read group.
     */
    public static String getPlatformUnit( final GaeaSamRecord read, final SAMFileHeader header ) {
        final SAMReadGroupRecord readGroup = getSAMReadGroupRecord(read, header);
     ...
     *         or null if the read has no read group.
     */
    public static String getLibrary( final GaeaSamRecord read, final SAMFileHeader header ) {
        final SAMReadGroupRecord readGroup = getSAMReadGroupRecord(read, header);
     ...
     *         or the read's unclipped end if the read is on the reverse strand.
     */
    public static int getStrandedUnclippedStart( final GaeaSamRecord read ) {
        return read.getReadNegativeStrandFlag() ? read.getUnclippedEnd() : read.getUnclippedStart();
    }
     ...
     */
    public static boolean readHasMappedMate( final GaeaSamRecord read ) {
        return read.getReadPairedFlag() && ! read.mateIsUnmapped();
    }
     ...
     * @throws UserException.ReadAttributeTypeMismatch if the value provided for that attribute is not an integer.
    ...
    public static OptionalInt getOptionalIntAttribute(final SAMRecord read, final String tag) {
        ...
        if (obj == null) {
            ...
            try {
                ...
                throw new UserException.ReadAttributeTypeMismatch(read, tag, "integer", ex);
            }
        } else {
            throw new UserException.ReadAttributeTypeMismatch(read, tag, "integer", obj);
        }
    }
     ...
     * @return never {@code null}, but perhaps empty indicating that no value was provided for this attribute.
     */
    public static OptionalInt getOptionalIntAttribute(final GaeaSamRecord read, final String tag) {
        Utils.nonNull(read);
        Utils.nonNull(tag);
    ...
    /**
     * Construct a set of SAM bitwise flags from a GaeaSamRecord
     *
    ...
    public static int getSAMFlagsForRead( final GaeaSamRecord read ) {
        ...
        if ( read.getReadPairedFlag() && read.mateIsUnmapped() ) {
            samFlags |= SAM_MATE_UNMAPPED_FLAG;
        }
        if ( !read.isUnmapped() && read.getReadNegativeStrandFlag() ) {
            samFlags |= SAM_READ_STRAND_FLAG;
        }
        if ( read.getReadPairedFlag() && ! read.mateIsUnmapped() && read.getMateNegativeStrandFlag() ) {
...
//            return false;
     ...
     */
    public static int getFirstInsertionOffset(final GaeaSamRecord read) {
     ...
     */
    public static int getLastInsertionOffset(final GaeaSamRecord read) {
     ...
     */
    public static int getSoftEnd(final GaeaSamRecord read) {
        Utils.nonNull(read, "read");
    ...
    public static int getReadCoordinateForReferenceCoordinateUpToEndOfRead(final GaeaSamRecord read, final int refCoord, final ClippingTail tail) {
     ...
     */
    public static int getReadCoordinateForReferenceCoordinate(final GaeaSamRecord read, final int refCoord, final ClippingTail tail) {
    ...
    private static Pair<Integer, Boolean> getReadCoordinateForReferenceCoordinate(final int alignmentStart, final Cigar cigar, final int refCoord, final boolean allowGoalNotReached) {
        ...
        if (goal < 0) {
            if (allowGoalNotReached) {
                ...
                throw new UserException("Somehow the requested coordinate is not covered by the read. Too many deletions?");
            }
        }
            ...
            if (goalReached) {
                ...
                if (!endsWithinCigar && !cigarElementIterator.hasNext()) {
                    if (allowGoalNotReached) {
                        ...
                        throw new UserException(String.format("Reference coordinate corresponds to a non-existent base in the read. This should never happen -- check read with alignment start: %s  and cigar: %s", alignmentStart, cigar));
                    }
                }
        ...
        if (!goalReached) {
            if (allowGoalNotReached) {
                ...
                throw new UserException("Somehow the requested coordinate is not covered by the read. Alignment " + alignmentStart + " | " + cigar);
            }
        }
    ...
    public static String getBasesReverseComplement(final byte[] bases) {
        ...
        for (int i = bases.length-1; i >=0; i--) {
            reverse += (char) BaseUtils.simpleComplement(bases[i]);
        }
        return reverse;
    }
     ...
     * @return the reverse complement of the read bases
     */
    public static String getBasesReverseComplement(final GaeaSamRecord read) {
     ...
     */
    public static int getMaxReadLength( final List<GaeaSamRecord> reads ) {
     ...
     */
    public static byte[] getExistingBaseInsertionQualities(final GaeaSamRecord read) {
     ...
     * @return the base deletion quality or null if read doesn't have one
     */
    public static byte[] getExistingBaseDeletionQualities(final GaeaSamRecord read) {
     ...
     */
    public static byte[] getBaseInsertionQualities(final GaeaSamRecord read) {
     ...
     */
    public static byte[] getBaseDeletionQualities(final GaeaSamRecord read) {
    ...
    public static byte[] getBaseQualities( final GaeaSamRecord read, final EventType errorModel ) {
        switch( errorModel ) {
            case SNP:
            ...
            default:
                throw new UserException("Unrecognized Base Recalibration type: " + errorModel );
        }
    }
     ...
     */
    public static GaeaSamRecord resetOriginalBaseQualities(final GaeaSamRecord read) {
        ...
        final boolean createMD5)
    {
        ...
        return ReadUtils.createCommonSAMWriterFromFactory(factory, outputPath.toFile(), referenceFile, header, preSorted);
    }
   ...
   public static SAMFileWriter createCommonSAMWriterFromFactory(
            ...
            final boolean preSorted)
    {
        return createCommonSAMWriterFromFactory(factory,
            Utils.nonNull(outputFile), referenceFile, header, preSorted);
    }
    ...
    public static boolean isNonPrimary(GaeaSamRecord read) {
        return read.getNotPrimaryAlignmentFlag() || read.getSupplementaryAlignmentFlag() || read.isUnmapped();
    }
        ...
        if (expectedSortOrder != SAMFileHeader.SortOrder.unsorted &&
            ...
            if (assumeSorted) {
                ...
                throw new UserException(
                        message +
                                "If you believe the file to be sorted correctly"
                );
            }
        }
     ...
     * </p>
     * <p>
     *     Notice that this index makes reference to the offset of that first base in the array returned by {@link GaeaSamRecord#getBases()}, If you
     ...
     *  against the reference (e.g. is unmapped). You can use {@link #hasBasesAlignedAgainstTheReference(GaeaSamRecord)} to check on
     *  the latter requirement.
     *
     * @return a number between 0 and the read length-1.
     */
    public static int getFirstAlignedBaseOffset(final GaeaSamRecord read) {
     ...
     */
    public static boolean hasBasesAlignedAgainstTheReference(final GaeaSamRecord read) {
        Utils.nonNull(read, "the input read cannot be null");
        if (read.isUnmapped()) {
            return false;
        } else {
     ...
     * <p>
     *     If what you are after is the offset of the first aligned base in the array returned by {@link GaeaSamRecord#getBases()}
     *     then you should be looking into {@link #getFirstAlignedBaseOffset(GaeaSamRecord)}.
     * </p>
     *
     ...
     *  the reference (e.g. is unmapped). You can use {@link #hasBasesAlignedAgainstTheReference(GaeaSamRecord)} to check on
     *  the latter requirement.
     *
     * @return a value between 1 and the length of the original read. Never {@code 0} nor a negative value.
     */
    public static int getFirstAlignedReadPosition(final GaeaSamRecord read) {
        Utils.nonNull(read, "the input read cannot be null");
        if (read.isUnmapped()) {
            throw new IllegalArgumentException("the input read is unmapped and therefore does not have any base aligned");
        } else {
            ...
            if (cigarElements.isEmpty()) {
                ...
                return read.getReadLength() - getAfterLastAlignedBaseOffset(read) + 1 + CigarUtils.countRightHardClippedBases(read.getCigar());
            }
        }
    }
     ...
     *     {@link #getLastAlignedReadPosition(GaeaSamRecord)} instead.
     * </p>
     *
     * @param read the query read.
     * @throws IllegalArgumentException if {@code read} is {@code null} or does not have any base aligned against the
     *    reference. You can use {@link #hasBasesAlignedAgainstTheReference(GaeaSamRecord)} to check on
     *  the latter requirement.
     *
     * @return a number between 0 and the read's length inclusive.
     */
    public static int getAfterLastAlignedBaseOffset(final GaeaSamRecord read) {
        if (read.isUnmapped()) {
            throw new IllegalArgumentException("the input read is unmapped and therefore does not have any base aligned");
        } else {
            final List<CigarElement> cigarElements = read.getCigar().getCigarElements();
     ...
     */
    public static int getLastAlignedReadPosition(final GaeaSamRecord read) {
        if (read.isUnmapped()) {
            ...
            if (cigarElements.isEmpty()) {
                ...
                return read.getReadLength() - offset + CigarUtils.countRightHardClippedBases(read.getCigar());
            }
        }
    }
