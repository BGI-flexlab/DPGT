...
public abstract class PairHMM implements Closeable{
     ...
     * @param readMaxLength the max length of reads we want to use with this PairHMM
     */
    public void initialize( final List<Haplotype> haplotypes, final Map<String, List<GaeaSamRecord>> perSampleReadList, final int readMaxLength, final int haplotypeMaxLength ) {
    ...
    static int findMaxReadLength(final List<GaeaSamRecord> reads) {
    ...
    public void computeLog10Likelihoods(final LikelihoodMatrix<Haplotype> logLikelihoods,
        ...
        int idx = 0;
        int readIndex = 0;
        for(final GaeaSamRecord read : processedReads){
    ...
    double computeReadLikelihoodGivenHaplotypeLog10( final byte[] haplotypeBases,
        ...
        Utils.validateArg(initialized, "Must call initialize before calling computeReadLikelihoodGivenHaplotypeLog10");
        ...
        Utils.validateArg(result <= 0.0, () -> "PairHMM Log Probability cannot be greater than 0: " + String.format("haplotype: %s, read: %s, result: %f, PairHMM: %s", new String(haplotypeBases), new String(readBases), result, this.getClass().getSimpleName()));
        Utils.validateArg(MathUtils.goodLog10Probability(result), () -> "Invalid Log Probability: " + result);
