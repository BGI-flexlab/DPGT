...
public class Histogram {
    ...
    public void add(final Double d) {
        ...
        if (isValidBinKey(binKey)) {
            ...
            throw new UserException("Histogram values are suspiciously extreme.  Failed to add " + d + " to the Histogram.");
        }
    }
    ...
    public void add(final Double d, final int count) {
        if (count < 1) {
            throw new UserException("Cannot add non-positive counts to Histogram.");
        }
        long binKey = getBinnedValue(d);
        if (isValidBinKey(binKey)) {
    ...
    public void add(final Histogram h) {
        if (!this.binSize.equals(h.binSize)) {
            throw new UserException("Histogram bin sizes are mismatched -- cannot add bin size " + this.binSize + " to " + h.binSize);
        }
    ...
    public Integer get(final Double d) {
        ...
        if (isValidBinKey(binKey)) {
            ...
            throw new UserException("Requested value is suspiciously extreme.  Failed to retrieve " + d + " from the Histogram.");
        }
    }
