...
public final class ReadFilterLibrary {
	...
	/**
	 * Filter out reads containing skipped region from the reference (CIGAR
	 * strings with 'N' operator).
	 */
	...
	public static class CigarContainsNoNOperator extends ReadFilter {
		...
		@Override
		public boolean test(final GaeaSamRecord read) {
			return !CigarUtils.containsNOperator(read.getCigar().getCigarElements());
		}
	}
	 ...
	 *
	 * <ul>
	 * <li>Valid according to the
	 * <a href="http://samtools.github.io/hts-specs/SAMv1.pdf">SAM
	 * specifications.</a></li>
	 * <li>Does not start or end with deletions (with or without preceding
	 * clips).</li>
	...
	public static class NonZeroFragmentLengthReadFilter extends ReadFilter {
		...
		@Override
		public boolean test(final GaeaSamRecord read) {
			return read.getInferredInsertSize() != 0;
		}
	}
	...
	public static class MatchingBasesAndQualsReadFilter extends ReadFilter {
		...
		@Override
		public boolean test(final GaeaSamRecord read) {
			return read.getReadLength() == (read.getBaseQualities() != null ? read.getBaseQualities().length : 0);
		}
	}
	 ...
	 *
	 * <p>
	 * Unmapped reads are defined by three criteria:
	 * </p>
	 *
	 * <ul>
	 * <li>SAM flag value 0x4</li>
	 * <li>An asterisk for reference name or RNAME (column 3 of a SAM
	 * record)</li>
	 * <li>A zero value for leftmost mapping position for POS (column 4 of SAM
	 * record)</li>
	 * </ul>
	 */
	...
	/**
	 * Keep only reads that have a mate that maps to the same contig (RNEXT is
	 * "="), is single ended (not 0x1) or has an unmapped mate (0x8).
	 *
	 * <p>
	...
	/**
	 * For paired reads (0x1), keep only reads that are mapped, have a mate that
	 * is mapped (read is not 0x8), and both the read and its mate are on
	 * different strands (when read is 0x20, it is not 0x10), as is the typical
	 * case.
	 *
	 * <p>
	...
	public static class MateDifferentStrandReadFilter extends ReadFilter {
		...
		@Override
		public boolean test(final GaeaSamRecord read) {
			return read.getReadPairedFlag() && !read.isUnmapped() && !read.mateIsUnmapped()
					&& read.getMateNegativeStrandFlag() != read.getReadNegativeStrandFlag();
		}
	}

	/**
	 * Filter out reads that do not align to the reference. Filter interprets
	 * each of the CIGAR operators M, D, N, = and X as alignment.
	 */
	...
	/**
	 * Keep only paired reads that are properly paired (0x1 and 0x2). Removes
	 * single ended reads.
	 */
	...
	/**
	 * Keep only reads representing primary alignments (those that satisfy both
	 * the NotSecondaryAlignment and NotSupplementaryAlignment filters, or in
	 * terms of SAM flag values, must have neither of the 0x100 or 0x800 flags
	 * set).
	 *
	 * <p>
	 * Note that this filter represents a stronger criteria for "primary
	 * alignment" than the SAM flag 0x100 (representing ""not primary alignment"
	 * in some contexts).
	 * </p>
	 *
	 * <p>
	 * For example, a read that has only the supplementary flag (0x800) set, but
	 * not the secondary (0x100) flag will be filtered out from processing by
	 * the PrimaryLineReadFilter, but would NOT be filtered out by other
	 * software that uses the looser notion of "not primary" that only depends
	 * on the "secondary" flag being set.
	 * </p>
	 */
	public static class PrimaryLineReadFilter extends ReadFilter {
		...
		@Override
		public boolean test(final GaeaSamRecord read) {
			return !read.getNotPrimaryAlignmentFlag() && !read.getSupplementaryAlignmentFlag();
		}
	}
	 ...
	 *
	 * <p>
	 * Note: unmapped reads pass this filter. See MappedReadFilter for criteria
	 * defining an unmapped read.
	 * </p>
	 */
	// Note: do not call getCigar to avoid creation of new Cigar objects
	public static class ReadLengthEqualsCigarLengthReadFilter extends ReadFilter {
		...
		@Override
		public boolean test(final GaeaSamRecord read) {
			return read.isUnmapped() || read.getReadLength() == Cigar.getReadLength(read.getCigar().getCigarElements());
		}
	}
	...
	public static class SecondOfPairReadFilter extends ReadFilter {
		...
		@Override
		public boolean test(final GaeaSamRecord read) {
			return read.getReadPairedFlag() && read.getSecondOfPairFlag();
		}
	}
	...
	public static class SeqIsStoredReadFilter extends ReadFilter {
		...
		@Override
		public boolean test(final GaeaSamRecord read) {
			return read.getReadLength() > 0;
		}
	}

	/**
	 * Keep only reads with a valid alignment start (POS larger than 0) or is
	 * unmapped.
	 *
	 * <p>
	 * See MappedReadFilter for criteria defining an unmapped read.
	...
	/**
	 * Keep only reads where the read end corresponds to a proper alignment --
	 * that is, the read ends after the start (non-negative number of bases in
	 * the reference).
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Note: keep also unmapped reads (align to zero bases in the reference).
	 * See MappedReadFilter for criteria defining an unmapped read.
	 * </p>
	 */
