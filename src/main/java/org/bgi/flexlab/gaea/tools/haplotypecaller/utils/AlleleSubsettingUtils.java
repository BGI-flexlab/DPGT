...
import org.bgi.flexlab.gaea.tools.haplotypecaller.allele.Permutation;
...
public final class AlleleSubsettingUtils {
        ...
        for (final Genotype g : originalGs) {
            ...
            if (g.hasLikelihoods()) {
                ...
                newLikelihoods = originalLikelihoods.length == expectedNumLikelihoods ?
                        GvcfMathUtils.scaleLogSpaceArrayForNumericalStability(Arrays.stream(subsettedLikelihoodIndices)
            ...
            final boolean useNewLikelihoods = newLikelihoods != null && (depth != 0 || GaeaGvcfVariantContextUtils.isInformative(newLikelihoods));
            ...
            GaeaGvcfVariantContextUtils.makeGenotypeCall(g.getPloidy(), gb, assignmentMethod, newLikelihoods, allelesToKeep);
    ...
    private static int[] getSACs(final Genotype g) {
        ...
        Class<?> clazz = g.getExtendedAttributes().get(GaeaVCFConstants.STRAND_COUNT_BY_SAMPLE_KEY).getClass();

        if ( clazz.equals(String.class) ) {
            final String SACsString = (String) g.getExtendedAttributes().get(GaeaVCFConstants.STRAND_COUNT_BY_SAMPLE_KEY);
    ...
    public static List<Allele> calculateMostLikelyAlleles(final VariantContext vc, final int defaultPloidy,
        ...
        final boolean hasSymbolicNonRef = vc.hasAllele(GaeaVCFConstants.NON_REF_SYMBOLIC_ALLELE);
    ...
    static List<Allele> filterToMaxNumberOfAltAllelesBasedOnScores(int numAltAllelesToKeep, List<Allele> alleles, double[] likelihoodSums) {
        final int nonRefAltAlleleIndex = alleles.indexOf(GaeaVCFConstants.NON_REF_SYMBOLIC_ALLELE);
