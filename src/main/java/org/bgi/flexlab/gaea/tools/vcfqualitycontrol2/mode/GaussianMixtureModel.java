...
import org.bgi.flexlab.gaea.tools.vcfqualitycontrol2.util.VCFQualityControlUtil;
...
public class GaussianMixtureModel {
    ...
    public void initializeRandomModel( final List<VariantDatum> data, final int numKMeansIterations ) {
        ...
        for( final MultivariateGaussian gaussian : gaussians ) {
            gaussian.initializeRandomMu( RandomDataGenertorUtils.getRandomGenerator() );
        }
        ...
        for( final MultivariateGaussian gaussian : gaussians ) {
            ...
            gaussian.initializeRandomSigma( RandomDataGenertorUtils.getRandomGenerator() );
    ...
    public void expectationStep( final List<VariantDatum> data ) {
        ...
        for( final VariantDatum datum : data ) {
            ...
            final double[] pVarInGaussianNormalized = VCFQualityControlUtil.normalizeLog10DeleteMePlease( pVarInGaussianLog10, false);
            int gaussianIndex = 0;
            for( final MultivariateGaussian gaussian : gaussians ) {
    ...
    public double normalizePMixtureLog10() {
        ...
        VCFQualityControlUtil.normalizeLog10DeleteMePlease( pGaussianLog10, true);
    ...
    public Double evaluateDatumInOneDimension( final VariantDatum datum, final int iii ) {
        ...
        int gaussianIndex = 0;
        for( final MultivariateGaussian gaussian : gaussians ) {
            pVarInGaussianLog10[gaussianIndex++] = gaussian.pMixtureLog10 + VCFQualityControlUtil.normalDistributionLog10(gaussian.mu[iii], gaussian.sigma.get(iii, iii), datum.annotations[iii]);
        }
        return nanTolerantLog10SumLog10(pVarInGaussianLog10); // Sum(pi_k * p(v|n,k))
    }
        ...
        for( int iii = 0; iii < datum.annotations.length; iii++ ) {
            ...
            if( datum.isNull[iii] ) {
                for( int ttt = 0; ttt < numIterPerMissingAnnotation; ttt++ ) {
                    datum.annotations[iii] = RandomDataGenertorUtils.getRandomGenerator().nextGaussian(); // draw a random sample from the standard normal distribution
    ...
    public List<MultivariateGaussian> getModelGaussians() {return Collections.unmodifiableList(gaussians);}

    public int getNumAnnotations() {return empiricalMu.length;}
}
