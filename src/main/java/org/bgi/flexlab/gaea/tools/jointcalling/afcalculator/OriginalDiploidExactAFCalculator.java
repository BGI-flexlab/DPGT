...
public class OriginalDiploidExactAFCalculator extends DiploidExactAFCalculator{
    ...
    protected AFCalculationResult computeLog10PNonRef(final VariantContext vc,
        ...
        final double log10LikelihoodAFGt0 = lastK == 0 ? GvcfMathUtils.LOG10_P_OF_ZERO : GvcfMathUtils.log10sumLog10(log10AlleleFrequencyLikelihoods, 1, lastK+1);
        final double[] log10Likelihoods = new double[]{log10AlleleFrequencyLikelihoods[0], log10LikelihoodAFGt0};
        final double[] log10Priors = new double[]{log10AlleleFrequencyPriors[0], GvcfMathUtils.log10sumLog10(log10AlleleFrequencyPriors, 1)};
        final double[] log10Posteriors = GvcfMathUtils.vectorSum(log10Likelihoods, log10Priors);

        final double log10PRef = log10Posteriors[1] > log10Posteriors[0] ? GvcfMathUtils.LOG10_P_OF_ZERO : 0.0;
        ...
        return new AFCalculationResult(new int[]{mleK}, 0, vc.getAlleles(),
        		GvcfMathUtils.normalizeFromLog10(log10Likelihoods, true),
        		GvcfMathUtils.normalizeFromLog10(log10Priors, true),
                log10pRefByAllele);
    }
        ...
        for (int k=0; k <= numChr && ! done; k++ ) {
            ...
            if ( k == 0 ) { // special case for k = 0
                ...
                for ( int j=1; j <= numSamples; j++ ) {
                    ...
                    if (k < 2*j-1)
                        aa = GvcfMathUtils.Log10Cache.get(2*j-k) + GvcfMathUtils.Log10Cache.get(2*j-k-1) + kMinus0[j-1] + gl[0];

                    if (k < 2*j)
                        ab = GvcfMathUtils.Log10Cache.get(2*k) + GvcfMathUtils.Log10Cache.get(2*j-k)+ kMinus1[j-1] + gl[1];

                    double log10Max;
                    if (k > 1) {
                        final double bb = GvcfMathUtils.Log10Cache.get(k) + GvcfMathUtils.Log10Cache.get(k-1) + kMinus2[j-1] + gl[2];
                        log10Max = GvcfMathUtils.approximateLog10SumLog10(aa, ab, bb);
                    } else {
                        // we know we aren't considering the BB case, so we can use an optimized log10 function
                        log10Max = GvcfMathUtils.approximateLog10SumLog10(aa, ab);
                    }
