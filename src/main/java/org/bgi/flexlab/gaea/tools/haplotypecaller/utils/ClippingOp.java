...
public final class ClippingOp {
     ...
     */
    public GaeaSamRecord apply(final ClippingRepresentation algorithm, final GaeaSamRecord originalRead) {
		return apply(algorithm, originalRead, true);
    }

    // Warning, this method is intended for testing purposes
    GaeaSamRecord apply(final ClippingRepresentation algorithm, final GaeaSamRecord originalRead, final boolean runAsserts) {
        switch (algorithm) {
            // important note:
            ...
            case WRITE_NS: {
                GaeaSamRecord readCopied = null;
				try {
					readCopied = (GaeaSamRecord)originalRead.clone();
            ...
            case SOFTCLIP_BASES: {
                ...
                return applySOFTCLIP_BASES(readCopied, runAsserts);
            }
            ...
            case REVERT_SOFTCLIPPED_BASES: {
                ...
                return applyREVERT_SOFTCLIPPED_BASES(readCopied);
            }
    ...
    private GaeaSamRecord applySOFTCLIP_BASES(final GaeaSamRecord readCopied, final boolean runAsserts) {
        ...
        int myStop = stop;
        if ((stop + 1 - start) == readCopied.getReadLength()) {
            // BAM representation issue -- we can't SOFTCLIP away all bases in a read, just leave it alone
            myStop--; // just decrement stop
        }

        if (start > 0 && myStop != readCopied.getReadLength() - 1) {
            throw new UserException(String.format("Cannot apply soft clipping operator to the middle of a read: %s to be clipped at %d-%d", readCopied.getReadName(), start, myStop));
        }

        final Cigar oldCigar = readCopied.getCigar();

        int scLeft = 0;
        int scRight = readCopied.getReadLength();
        if (start == 0) {
            scLeft = myStop + 1;
        } else {
            scRight = start;
    ...
    private void applyWRITE_Q0S(final GaeaSamRecord readCopied) {
    ...
    private void applyWRITE_NS(final GaeaSamRecord readCopied) {
        final byte[] newBases = readCopied.getReadBases();       //this makes a copy so we can modify in place
        overwriteFromStartToStop(newBases, (byte)'N');
        readCopied.setReadBases(newBases);
    }
    ...
    private GaeaSamRecord applyREVERT_SOFTCLIPPED_BASES(final GaeaSamRecord read) {
    	GaeaSamRecord unclipped = null;
		try {
        ...
        int matchesCount = 0;
        for (final CigarElement element : Collections.unmodifiableList(read.getCigar().getCigarElements())) {
        ...
        if ( newStart <= 0 ) {
            ...
            // negative or 0 alignment start values in the GaeaSamRecord (e.g., 0 means unaligned)
            ...
            // that cache these values, such as GaeaSamRecord), and again after the hard clip.
     ...
     */
    private GaeaSamRecord applyHARDCLIP_BASES(final GaeaSamRecord read, final int start, final int stop) {
        ...
        GaeaSamRecord hardClippedRead = null;
		try {
        ...
        if (start == 0 && !read.getReadUnmappedFlag()) {
        ...
        if (start == 0) {
            ...
            while (cigarElement.getOperator() == CigarOperator.HARD_CLIP) {
                ...
                if (cigarElementIterator.hasNext()) {
                    ...
                    throw new UserException("Read is entirely hardclipped, shouldn't be trying to clip it's cigar string");
                }
            }
