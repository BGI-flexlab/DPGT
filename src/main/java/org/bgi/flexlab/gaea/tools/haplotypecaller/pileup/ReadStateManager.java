...
final class ReadStateManager implements Iterable<Map.Entry<String, PerSampleReadStateManager>> {
    ...
    private final PeekableIterator<GaeaSamRecord> iterator;
    ...
    private List<GaeaSamRecord> submittedReads;
    private final boolean keepSubmittedReads;

    private int totalReadStates = 0;

    public ReadStateManager(final Iterator<GaeaSamRecord> source,
     ...
     */
    private boolean readStartsAtCurrentPosition(final GaeaSamRecord read, final String currentContig, final int currentAlignmentStart) {
    ...
    public void collectPendingReads() {
        ...
        for (final String sample : samples) {
            final Collection<GaeaSamRecord> newReads = samplePartitioner.getReadsForSample(sample);
     ...
     */
    void submitRead(final GaeaSamRecord read) {
     ...
     */
    public List<GaeaSamRecord> transferSubmittedReads() {
        Utils.validateArg(keepSubmittedReads,"cannot transferSubmittedReads if you aren't keeping them");
        final List<GaeaSamRecord> prevSubmittedReads = submittedReads;
        this.submittedReads = new LinkedList<>();
     ...
     */
    @VisibleForTesting
    List<GaeaSamRecord> getSubmittedReads() {
     ...
     */
    private void addReadsToSample(final PerSampleReadStateManager readStates, final Collection<GaeaSamRecord> reads) {
        if (reads.isEmpty()) {
            return;
        }
