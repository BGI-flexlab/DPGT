...
import org.bgi.flexlab.gaea.tools.haplotypecaller.pileup.AlignmentStateMachine;
...
public class GaeaProtectedVariantContextUtils {
            ...
            if (o == null) {
                ...
                if (s.equals(VCFConstants.MISSING_VALUE_v4)) {
                    ...
                    try {
                        ...
                        throw new UserException(String.format("INFO annotation '%s' contains a non-double value '%s'", key, s), ex);
                    }
                }
            }
        };
            ...
            if (o == null) {
                ...
                if (s.equals(VCFConstants.MISSING_VALUE_v4)) {
                    ...
                    try {
                        ...
                        throw new UserException(String.format("INFO annotation '%s' contains a non-int value '%s'", key, s), ex);
                    }
                }
            }
        };
    ...
    public static double calculateGenotypeQualityFromPLs(final Genotype genotype) {
        ...
        if (!genotype.hasPL()) {
            ...
            final int GQ = GaeaProtectedMathUtils.secondSmallestMinusSmallest(genotype.getPL(), -1);
     ...
     */
    public static ReadPileup getPileup(final Locatable loc, final Iterable<GaeaSamRecord> reads) {
    ...
    private static byte[] getBasesForAlleleInRead(final PileupElement pileupElement, final Allele allele) {
        Utils.nonNull(pileupElement);
        Utils.nonNull(allele);
        return ArrayUtils.subarray(pileupElement.getRead().getReadBases(), pileupElement.getOffset(), pileupElement.getOffset() + allele.getBases().length);
    }
    ...
    public static Trilean doesReadContainAllele(final PileupElement pileupElement, final Allele allele) {
        ...
        final byte[] readBases = ArrayUtils.subarray(pileupElement.getRead().getReadBases(), pileupElement.getOffset(), pileupElement.getOffset() + allele.getBases().length);
