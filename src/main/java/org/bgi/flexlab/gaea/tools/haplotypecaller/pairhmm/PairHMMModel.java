...
public final class PairHMMModel {
     ...
     */
    private static final double[] matchToMatchProb = new double[((QualityUtils.MAXIMUM_QUALITY_SCORE + 1) * (QualityUtils.MAXIMUM_QUALITY_SCORE + 2)) >> 1];
     ...
     * (0,0), (0,1), (1,1), (0,2), (1,2), (2,2), (0,3) ... ({@link QualityUtils#MAX_QUAL},{@link QualityUtils#MAX_QUAL})
     */
    private static final double[] matchToMatchLog10 = new double[((QualityUtils.MAXIMUM_QUALITY_SCORE + 1) * (QualityUtils.MAXIMUM_QUALITY_SCORE + 2)) >> 1];

    /**
     * Initialize matchToMatch cache tables {@link #matchToMatch} and {@link #matchToMatchLog}
     */
    static {
        for (int i = 0, offset = 0; i <= QualityUtils.MAXIMUM_QUALITY_SCORE; offset += ++i)
            for (int j = 0; j <= i; j++) {
                final double log10Sum = GvcfMathUtils.approximateLog10SumLog10(-0.1 * i, -0.1 * j);
    ...
    public static double matchToMatchProb(final int insQual, final int delQual) {
        ...
        return (QualityUtils.MAXIMUM_QUALITY_SCORE < maxQual) ?  1.0 - Math.pow(10, GvcfMathUtils.approximateLog10SumLog10(-0.1 * minQual, -0.1 * maxQual)) :
    ...
    public static double matchToMatchProbLog10(final int insQual, final int delQual) {
        ...
        return (QualityUtils.MAXIMUM_QUALITY_SCORE < maxQual) ? Math.log1p (
                -Math.min(1, Math.pow(10,
                        GvcfMathUtils.approximateLog10SumLog10(-.1 * minQual, -.1 * maxQual)))) * INV_LN10 :
