...
public class VariantDataManager {
	...
	private final VCFQualityControlArgumentCollection VRAC;
	protected final List<TrainData> trainingSets;
	private static final double SAFETY_OFFSET = 0.01; // To use for example as
														// 1/(X + SAFETY_OFFSET)
														// to protect against
														// dividing or taking
														// log of X=0.
	private static final double PRECISION = 0.01; // To use mainly with
													// MathUtils.compareDoubles(a,b,PRECISION)

	public VariantDataManager(final List<String> annotationKeys, final VCFQualityControlArgumentCollection VRAC) {
	...
	public void normalizeData(final boolean calculateMeans) {
		...
		for (int iii = 0; iii < meanVector.length; iii++) {
			...
			for (final VariantDatum datum : data) {
				// Transform each data point via: (x - mean) / standard
				// deviation
				datum.annotations[iii] = (datum.isNull[iii]
						? 0.1 * RandomDataGenertorUtils.getRandomGenerator().nextGaussian()
		...
		// trim data by standard deviation threshold and mark failing data for
		// exclusion later
		for (final VariantDatum datum : data) {
		...
		// re-order the data by increasing standard deviation so that the
		// results don't depend on the order things were specified on the
		// command line
		// standard deviation over the training points is used as a simple proxy
		// for information content, perhaps there is a better thing to use here
	...
	/**
	 * Get a list of indices which give the ascending sort order of the data
	 * array
	 * 
	 * @param inputVector
	 *            the data to consider
	 * @return a non-null list of integers with length matching the length of
	 *         the input array
	 */
	protected List<Integer> calculateSortOrder(final double[] inputVector) {
		...
		for (final MyDoubleForSorting d : toBeSorted) {
			theOrder.add(d.originalIndex); // read off the sort order by looking
											// at the index field
		}
		return theOrder;
	}

	// small private class to assist in reading off the new ordering of the
	// annotation array
	...
	/**
	 * Convenience connector method to work with arrays instead of lists. See
	 * ##reorderList##
	 */
	 ...
	 *
	 * norm = (orig - mu) / sigma orig = norm * sigma + mu
	 *
	 * @param normalizedValue
	 *            the normalized value of the ith annotation
	 * @param annI
	 *            the index of the annotation value
	...
	public List<VariantDatum> getTrainingData() {
		...
		if (trainingData.size() < VRAC.MIN_NUM_BAD_VARIANTS) {
			...
			Collections.shuffle(trainingData, RandomDataGenertorUtils.getRandomGenerator());
	...
	/**
	 * Remove all VariantDatum's from the data list which are marked as
	 * aggregate data
	 */
	...
	public List<VariantDatum> getRandomDataForPlotting(final int numToAdd, final List<VariantDatum> trainingData,
			final List<VariantDatum> antiTrainingData, final List<VariantDatum> evaluationData) {
		final List<VariantDatum> returnData = new ExpandingArrayList<>();
		Collections.shuffle(trainingData, RandomDataGenertorUtils.getRandomGenerator());
		Collections.shuffle(antiTrainingData, RandomDataGenertorUtils.getRandomGenerator());
		Collections.shuffle(evaluationData, RandomDataGenertorUtils.getRandomGenerator());
		...
		Collections.shuffle(returnData, RandomDataGenertorUtils.getRandomGenerator());
		return returnData;
	}
	...
	private static double decodeAnnotation(final String annotationKey, final VariantContext vc, final boolean jitter,
			final VCFQualityControlArgumentCollection vrac, final VariantDatum datum) {
		double value;

		final double LOG_OF_TWO = 0.6931472;

		try {
			// if we're in allele-specific mode and an allele-specific
			// annotation has been requested, parse the appropriate value from
			// the list
			if (vrac.useASannotations && annotationKey.startsWith(GaeaVCFConstants.ALLELE_SPECIFIC_PREFIX)) {
				final List<Object> valueList = vc.getAttributeAsList(annotationKey);
				if (vc.hasAllele(datum.alternateAllele)) {
					final int altIndex = vc.getAlleleIndex(datum.alternateAllele) - 1; // -1
																						// is
																						// to
																						// convert
																						// the
				...
				// if somehow our alleles got mixed up
					...
					throw new IllegalStateException("VariantDatum allele " + datum.alternateAllele
							+ " is not contained in the input VariantContext.");
			} else
				value = vc.getAttributeAsDouble(annotationKey, Double.NaN);
			if (Double.isInfinite(value)) {
				value = Double.NaN;
			}
			if (jitter && annotationKey.equalsIgnoreCase(GaeaVCFConstants.HAPLOTYPE_SCORE_KEY)
					&& MathUtils.compareDoubles(value, 0.0, PRECISION) == 0) {
				value += 0.01 * RandomDataGenertorUtils.getRandomGenerator().nextGaussian();
			}
			if (jitter
					&& (annotationKey.equalsIgnoreCase(GaeaVCFConstants.FISHER_STRAND_KEY)
							|| annotationKey.equalsIgnoreCase(GaeaVCFConstants.AS_FILTER_STATUS_KEY))
					&& MathUtils.compareDoubles(value, 0.0, PRECISION) == 0) {
				value += 0.01 * RandomDataGenertorUtils.getRandomGenerator().nextGaussian();
			}
			if (jitter && annotationKey.equalsIgnoreCase(GaeaVCFConstants.INBREEDING_COEFFICIENT_KEY)
					&& MathUtils.compareDoubles(value, 0.0, PRECISION) == 0) {
				value += 0.01 * RandomDataGenertorUtils.getRandomGenerator().nextGaussian();
			}
			if (jitter
					&& (annotationKey.equalsIgnoreCase(GaeaVCFConstants.STRAND_ODDS_RATIO_KEY)
							|| annotationKey.equalsIgnoreCase(GaeaVCFConstants.AS_STRAND_ODDS_RATIO_KEY))
					&& MathUtils.compareDoubles(value, LOG_OF_TWO, PRECISION) == 0) {
				value += 0.01 * RandomDataGenertorUtils.getRandomGenerator().nextGaussian();
			} // min SOR is 2.0, then we take ln
			if (jitter && (annotationKey.equalsIgnoreCase(VCFConstants.RMS_MAPPING_QUALITY_KEY)
					|| annotationKey.equalsIgnoreCase(GaeaVCFConstants.AS_RMS_MAPPING_QUALITY_KEY))) {
				if (vrac.MQ_CAP > 0) {
					value = logitTransform(value, -SAFETY_OFFSET, vrac.MQ_CAP + SAFETY_OFFSET);
					if (MathUtils.compareDoubles(value,
							logitTransform(vrac.MQ_CAP, -SAFETY_OFFSET, vrac.MQ_CAP + SAFETY_OFFSET), PRECISION) == 0) {
						value += vrac.MQ_JITTER * RandomDataGenertorUtils.getRandomGenerator().nextGaussian();
					}
			...
			value = Double.NaN; // The VQSR works with missing data by
								// marginalizing over the missing dimension when
								// evaluating the Gaussian mixture model
		}

		return value;
	}

	public void parseTrainingSets(final GenomeLocationParser genomeLocParser, final VariantContext evalVC,
			final VariantDatum datum, final boolean TRUST_ALL_POLYMORPHIC) {
		datum.isKnown = false;
		...
		GenomeLocation loc = genomeLocParser.createGenomeLocation(evalVC);

		for (final TrainData trainingSet : trainingSets) {
			List<VariantContext> vcs = trainingSet.get(loc);
			for (final VariantContext trainVC : vcs) {
				...
				if (isValidVariant(evalVC, trainVC, TRUST_ALL_POLYMORPHIC)) {
					...
					datum.prior = Math.max(datum.prior, trainingSet.getPrior());
				}
				if (trainVC != null) {
	...
	private boolean isValidVariant(final VariantContext evalVC, final VariantContext trainVC,
	...
	private boolean doAllelesMatch(final VariantContext trainVC, final VariantDatum datum) {
		// only do this check in the allele-specific case, where each datum
	...
	protected static boolean checkVariationClass(final VariantContext evalVC, final VariantContext trainVC) {
		...
		case SNP:
		case MNP:
			return checkVariationClass(evalVC, VCFQualityControlArgumentCollection.Mode.SNP);
		case INDEL:
		case MIXED:
		case SYMBOLIC:
			return checkVariationClass(evalVC, VCFQualityControlArgumentCollection.Mode.INDEL);
		default:
			return false;
		}
	}

	protected static boolean checkVariationClass(final VariantContext evalVC,
			final VCFQualityControlArgumentCollection.Mode mode) {
		switch (mode) {
		case SNP:
	...
	protected static boolean checkVariationClass(final VariantContext evalVC, final Allele allele,
			final VCFQualityControlArgumentCollection.Mode mode) {
		switch (mode) {
		case SNP:
