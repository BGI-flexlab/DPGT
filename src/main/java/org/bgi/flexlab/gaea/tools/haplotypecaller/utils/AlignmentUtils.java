...
public final class AlignmentUtils {
     ...
     *
     * @return a GaeaSamRecord aligned to reference. Never {@code null}.
     */
    public static GaeaSamRecord createReadAlignedToRef(final GaeaSamRecord originalRead,
        ...
        final SmithWatermanAlignment swPairwiseAlignment = aligner.align(haplotype.getBases(), originalRead.getReadBases(), new SWParameters(200, -150, -260, -11), SWOverhangStrategy.SOFTCLIP);
        ...
        final GaeaSamRecord read = originalRead.copy();
        ...
        final Cigar readToRefCigar = leftAlignIndel(readToRefCigarClean, refHaplotype.getBases(),
                                                    originalRead.getReadBases(), swPairwiseAlignment.getAlignmentOffset(), 0, true);

        read.setCigar(readToRefCigar);

        if ( readToRefCigar.getReadLength() != read.getReadLength() ) {
            throw new UserException("Cigar " + readToRefCigar + " with read length " + readToRefCigar.getReadLength()
                    + " != read length " + read.getReadLength() + " for read " + read.toString() + "\nhapToRef " + haplotypeToRef + " length " + haplotypeToRef.getReadLength() + "/" + haplotypeToRef.getReferenceLength()
        ...
        for (final CigarElement ce : cigar.getCigarElements()) {
            ...
            switch (ce.getOperator()) {
                ...
                case P:
                    break;
                default:
                    throw new UserException("The " + ce.getOperator() + " cigar element is not currently supported");
            }
        }
    ...
    /**
     * @see #getMismatchCount(GaeaSamRecord, byte[], int, int, int) with startOnRead == 0 and nReadBases == read.getReadLength()
     */
    public static MismatchCount getMismatchCount(GaeaSamRecord r, byte[] refSeq, int refIndex) {
     ...
     */
    @SuppressWarnings("fallthrough")
    public static MismatchCount getMismatchCount(GaeaSamRecord r, byte[] refSeq, int refIndex, int startOnRead, int nReadBases) {
        Utils.nonNull( r );
     ...
     */
    public static int getNumAlignmentBlocks(final GaeaSamRecord r) {
        Utils.nonNull( r );
     ...
     */
    public static int getNumAlignedBasesCountingSoftClips(final GaeaSamRecord r) {
        int n = 0;
        final Cigar cigar = r.getCigar();
        if (cigar == null) return 0;
     ...
     */
    public static int getNumHardClippedBases(final GaeaSamRecord r) {
     ...
     */
    public static int calcNumHighQualitySoftClips( final GaeaSamRecord read, final byte qualThreshold ) {
            ...
            switch (ce.getOperator()) {
                case I:
                    if (alignPos > 0) {
                        ...
                        if (alignment[prevPos] == BaseUtils.Base.A.getBase()) {
                            alignment[prevPos] = PileupElement.A_FOLLOWED_BY_INSERTION_BASE;
                        } else if (alignment[prevPos] == BaseUtils.Base.C.getBase()) {
                            alignment[prevPos] = PileupElement.C_FOLLOWED_BY_INSERTION_BASE;
                        } else if (alignment[prevPos] == BaseUtils.Base.T.getBase()) {
                            alignment[prevPos] = PileupElement.T_FOLLOWED_BY_INSERTION_BASE;
                        } else if (alignment[prevPos] == BaseUtils.Base.G.getBase()) {
    ...
    public static Cigar trimCigarByReference(final Cigar cigar, final int start, final int end) {
        ...
        Utils.validateArg(result.getReferenceLength() == end - start + 1, () -> "trimCigarByReference failure: start " + start + " end " + end + " for " + cigar + " resulted in cigar with wrong size " + result);
        return result;
    }
    ...
    public static Cigar trimCigarByBases(final Cigar cigar, final int start, final int end) {
        ...
        Utils.validateArg(result.getReadLength() == expectedSize, () -> "trimCigarByBases failure: start "
