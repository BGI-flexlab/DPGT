...
public final class PileupElement {
    ...
    private final GaeaSamRecord read;         // the read this base belongs to
     ...
     */
    public PileupElement(final GaeaSamRecord read,
        ...
        Utils.validIndex(baseOffset, read.getReadLength());
     ...
     */
    public static PileupElement createPileupForReadAndOffset(final GaeaSamRecord read, final int offset) {
        Utils.nonNull(read, "read is null");
        Utils.validIndex(offset, read.getReadLength());
    ...
    public String toString() {
        return String.format("%s @ %d = %c Q%d", read.getReadName(), offset, (char) getBase(), getQual());
    }
    ...
    CigarOperator getAdjacentOperator(final Direction direction) {
        ...
        if (i < 0 || i >= read.getCigar().numCigarElements()) {
            return null;
        }
        return read.getCigar().getCigarElement(i).getOperator();
    }
    /**
