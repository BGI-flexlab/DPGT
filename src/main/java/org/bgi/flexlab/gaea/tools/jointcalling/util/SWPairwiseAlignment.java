...
public class SWPairwiseAlignment {
    ...
    public SWPairwiseAlignment(byte[] seq1, byte[] seq2) {
       	this.parameters = NEW_SW_PARAMETERS;
        align(seq1,seq2);
    }
    ...
    protected SWPairwiseAlignmentResult calculateCigar(final int[][] sw, final int[][] btrack, final OVERHANG_STRATEGY overhang_strategy) {
        ...
        return new SWPairwiseAlignmentResult(AlignmentExtendUtils.consolidateCigar(new Cigar(lce)), alignment_offset);
    }
        ...
        for ( CigarElement e : cigar.getCigarElements() ) {
            switch (e.getOperator()) {
                ...
                case D:
                    ...
                    break;
                default:
                    throw new UserException("Unexpected Cigar element:" + e.getOperator());
            }
        }
