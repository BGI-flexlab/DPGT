...
import org.bgi.flexlab.gaea.data.exception.UserException.MalformedBAM;
...
public final class AlignmentStateMachine {
    ...
    public AlignmentStateMachine(final GaeaSamRecord read) {
        this.read = read;
    ...
    public GenomeLocation getLocation() {
        ...
        return new GenomeLocation(read.getContig(), getGenomePosition(), getGenomePosition());
    }
            ...
            if (currentElement == null || (offsetIntoCurrentCigarElement + 1) >= currentElement.getLength()) {
                ...
                if (currentCigarElementOffset < nCigarElements) {
                    ...
                    if (currentElement != null && currentElement.getOperator() == CigarOperator.D)
                        throw new MalformedBAM(read, "read ends with deletion. Cigar: " + read.getCigar().toString() + ". Although the SAM spec technically permits such reads, this is often indicative of malformed files.");
            ...
            switch (currentElement.getOperator()) {
                ...
                case D: // deletion w.r.t. the reference
                    if (readOffset < 0)             // we don't want reads starting with deletion, this is a malformed cigar string
                        throw new MalformedBAM(read, "read starts with deletion. Cigar: " + read.getCigar().toString() + ". Although the SAM spec technically permits such reads, this is often indicative of malformed files.");
    ...
    public final PileupElement makePileupElement() {
        Utils.validateArg(!(isLeftEdge() || isRightEdge()), "Cannot make a pileup element from an edge alignment state");
